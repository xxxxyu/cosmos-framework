...
The same commit was shallow-cloned on `4090-NX-1`, installed into a fresh
locked runtime, and run through `pipeline.sh rollout` with the policy on GPU 0
...
Internal provenance paths from the original validation machine follow. They
...
Internal smoke provenance (not a deployment input):
...
Internal paired-run provenance (not a deployment input):
