...
class TorchScaledMMFp8W8A8Linear(nn.Module):
    """Speed-only tensor-wise FP8 W8A8 prototype using torch._scaled_mm.

    This is not a calibrated deployment backend. It dynamically quantizes
    activations each forward and stores a tensor-wise FP8 weight copy in the
    column-major layout required by cuBLASLt on Ada.
    """
...
class TorchScaledMMFp8W8A8StaticScaleLinear(nn.Module):
    """Speed-only FP8 W8A8 with a fixed activation scale.

    This simulates a calibrated per-tensor activation scale. It removes the
    runtime amax reduction but still pays a separate eager quantization pass.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_2d = x.reshape(-1, x.shape[-1]).contiguous()
        q_x = (x_2d / self.scale_a).clamp(-self.fp8_max, self.fp8_max).to(torch.float8_e4m3fn)
        out = torch._scaled_mm(
            q_x,
            self.qweight_col_major,
...
class VllmCutlassFp8W8A8BaseLinear(nn.Module):
    def __init__(self, weight: torch.Tensor) -> None:
        super().__init__()
        if not torch.cuda.is_available() or weight.device.type != "cuda":
    ...
    def _cutlass_mm(self, q_x: torch.Tensor, scale_a: torch.Tensor, x_shape: torch.Size) -> torch.Tensor:
        ...
        return out.reshape(x_shape[:-1] + (self.size_n,)).to(torch.bfloat16)


class VllmCutlassFp8W8A8DynamicLinear(VllmCutlassFp8W8A8BaseLinear):
    """vLLM dynamic scaled_fp8_quant plus CUTLASS scaled-mm."""

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_2d = x.reshape(-1, x.shape[-1]).contiguous()
        q_x, scale_a = self._ops().scaled_fp8_quant(x_2d)
        return self._cutlass_mm(q_x, scale_a, x.shape)


class VllmCutlassFp8W8A8StaticScaleLinear(VllmCutlassFp8W8A8BaseLinear):
    """vLLM static scaled_fp8_quant plus CUTLASS scaled-mm."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_2d = x.reshape(-1, x.shape[-1]).contiguous()
        q_x, scale_a = self._ops().scaled_fp8_quant(x_2d, self.scale_a, group_shape=(-1, -1))
        return self._cutlass_mm(q_x, scale_a, x.shape)


class VllmCutlassFp8W8A8KernelOnlyLinear(VllmCutlassFp8W8A8BaseLinear):
    """Kernel-only vLLM CUTLASS scaled-mm with prequantized activation."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if tuple(x.shape) != self.input_shape:
            raise ValueError(f"kernel-only FP8 benchmark expected input shape {self.input_shape}, got {tuple(x.shape)}")
        return self._cutlass_mm(self.q_x, self.scale_a, x.shape)
...
class TorchInt8MMW8A8Linear(nn.Module):
    ...
    This is not a calibrated deployment backend. It dynamically quantizes
    activations each forward, performs INT8xINT8->INT32 matmul, then dequantizes
    through a separate pointwise operation.
    """

    def __init__(self, weight: torch.Tensor) -> None:
        super().__init__()
        if not torch.cuda.is_available() or weight.device.type != "cuda":
            raise RuntimeError("torch._int_mm backend requires CUDA tensors")
        if not hasattr(torch, "_int_mm"):
            raise RuntimeError("torch._int_mm is not available")
        self.size_n, self.size_k = weight.shape
        self.int8_max = 127.0
        weight_kn = weight.detach().to(torch.bfloat16).t().contiguous()
        scale_b = (weight_kn.abs().amax().float() / self.int8_max).clamp(min=1e-8)
        q_weight = torch.round(weight_kn / scale_b).clamp(-127, 127).to(torch.int8).contiguous()
        self.register_buffer("qweight", q_weight, persistent=False)
        self.register_buffer("scale_b", scale_b.reshape(1), persistent=False)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_2d = x.reshape(-1, x.shape[-1]).contiguous()
        scale_a = (x_2d.abs().amax().float() / self.int8_max).clamp(min=1e-8)
        q_x = torch.round(x_2d / scale_a).clamp(-127, 127).to(torch.int8).contiguous()
        out_i32 = torch._int_mm(q_x, self.qweight)
        out = out_i32.to(torch.float32) * (scale_a * self.scale_b)
        return out.reshape(x.shape[:-1] + (self.size_n,)).to(torch.bfloat16)
...
def _make_backend(name: str, weight: torch.Tensor, example_x: torch.Tensor | None = None) -> nn.Module:
    ...
    if name == "torch_scaled_mm_fp8_w8a8_static_scale":
        if example_x is None:
            raise ValueError("torch_scaled_mm_fp8_w8a8_static_scale requires example_x")
        return TorchScaledMMFp8W8A8StaticScaleLinear(weight, example_x)
    ...
    if name == "vllm_cutlass_fp8_w8a8_dynamic":
        return VllmCutlassFp8W8A8DynamicLinear(weight)
    if name == "vllm_cutlass_fp8_w8a8_static_scale":
        if example_x is None:
            raise ValueError("vllm_cutlass_fp8_w8a8_static_scale requires example_x")
        return VllmCutlassFp8W8A8StaticScaleLinear(weight, example_x)
    if name == "vllm_cutlass_fp8_w8a8_kernel_only":
        if example_x is None:
            raise ValueError("vllm_cutlass_fp8_w8a8_kernel_only requires example_x")
        return VllmCutlassFp8W8A8KernelOnlyLinear(weight, example_x)
...
def main() -> None:
    ...
    results: dict[str, object] = {
        ...
        "notes": {
            ...
            "torch_scaled_mm_fp8_w8a8_static_scale": "experimental speed-only tensor-wise FP8 W8A8; fixed activation scale; eager quantization; uses torch._scaled_mm",
            ...
            "vllm_cutlass_fp8_w8a8_dynamic": "experimental speed-only tensor-wise FP8 W8A8; vLLM dynamic scaled_fp8_quant plus CUTLASS scaled-mm",
            "vllm_cutlass_fp8_w8a8_static_scale": "experimental speed-only tensor-wise FP8 W8A8; fixed activation scale; vLLM static scaled_fp8_quant plus CUTLASS scaled-mm",
            "vllm_cutlass_fp8_w8a8_kernel_only": "experimental kernel-only tensor-wise FP8 W8A8; prequantized fixed benchmark activation; vLLM CUTLASS scaled-mm only",
